...
void SystODEs::FastSystem()
{
   ...
   U4 = kp4*(psit - psi) - kd4*dblDydt[4];
	...
	dblDydt[3] = (sin(psi)*sin(phi) + cos(psi)*sin(theta)*cos(phi))*dblInputs[0]/mass - kd*dbly[3];
	dblDydt[4] = (sin(psi)*sin(theta)*cos(phi) - cos(psi)*sin(phi))*dblInputs[0]/mass - kd*dbly[4];
	...
	dblDydt[10] = (dblInputs[2]  + p*r*(Izz - Ixx) + JTP*p*Omega) / Iyy;
	dblDydt[11] = (dblInputs[3]  + p*q*(Ixx - Iyy)) / Izz;
}
